...
const api =
	"https://www.zhihu.com/api/v4/columns/c_1715391799055720448/items?limit=2";
...
export async function fetch60s(type = "json", ctx: Context) {
	...
	const today = Math.trunc(
		(Date.now() + timeZoneOffset * oneHourMs) / (24 * oneHourMs),
	);

	if (!cache.get(today)) {
		const { data = [] } = await (
			await fetch(api, { headers: { cookie: `__zse_ck=${zseCk};` } })
		).json();
		const { content = "", url = "", title_image = "", updated = 0 } = data[0];
		const contents: string[] = content.match(reg) ?? [];
		const mapFn = (e: string) =>
			transferText(e.replace(tagReg, "").trim(), "a2u");
