...
;(async () => {
  ...
  if (match) {
    ...
    let response = await fetch(videoUrl, { redirect: 'manual', headers: { 'User-Agent': userAgent } })
    ...
    // )
    // console.log('----------------------------')
    response = await fetch(location!, { redirect: 'manual', headers: { 'User-Agent': userAgent } })
    ...
    // console.log(location)
    // console.log('----------------------------')
    const videoId = location!.split('?')[0].split('/').pop()
    // console.log(videoId)
    // console.log('----------------------------')
    ...
    // console.log(location, getCookieValue(cookies, '__ac_nonce'), userAgent)
    cookies = mergeCookies(
      cookies,
      `__ac_signature=${get__ac_signature(location!, getCookieValue(cookies, '__ac_nonce'), userAgent)}; __ac_referer=__ac_blank;`,
    )
    // console.log(cookies)
    // console.log('----------------------------')
    response = await fetch(location!, {
    ...
    console.log(
      (
        await (
          await fetch(url + '&a_bogus=' + get_ab(parameter, userAgent), {
            headers: {
              cookie: cookies,
              'User-Agent': userAgent,
