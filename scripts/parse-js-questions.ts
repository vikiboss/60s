...
async function fetchMarkdown(): Promise<string> {
  const response = await fetch('https://raw.githubusercontent.com/lydiahallie/javascript-questions/refs/heads/master/zh-CN/README-zh_CN.md')
...
function parseQuestionBlock(id: number, content: string): JavaScriptQuestion | null {
  ...
  // 提取答案和解释
  const detailsMatch = content.match(/<details><summary><b>答案<\/b><\/summary>\n<p>\n\n#### 答案：([A-Z])\n\n([\s\S]*?)\n\n<\/p>\n<\/details>/)
  ...
  if (detailsMatch) {
    ...
    explanation = detailsMatch[2].trim()
      // 清理HTML标签
      .replace(/<[^>]*>/g, '')
      // 清理多余空白
      .replace(/\n\s*\n/g, '\n\n')
...
async function main() {
  try {
    ...
    await Deno.writeTextFile(outputPath, JSON.stringify(questions, null, 2))
